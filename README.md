...
**把家里那台跑着 AI Coding Agent 的 Mac，装进你的手机里。**
...
OpenNexus 是一个**开源的「AI Coding Agent 移动控制台」**：让你在通勤、咖啡馆、床上，用手机继续指挥家里 Mac 上正在跑的 AI 编码助手，看代码、改代码、审批危险操作，全程**不需要打开电脑**。

> **当前支持**：Claude Code（v0.1）
> **路线图**：Codex CLI、OpenCode（v0.3）
...
| ------------------------------------------------------------ | -------------------------------- | ------------------------------------------------------------ |
| 出门在外，AI Agent 在 Mac 上跑到一半，**需要回复 / 决策** | 跑回家、远程桌面、SSH+tmux 手敲  | 手机收到推送 → **点按钮直接「允许 / 拒绝」**（PreToolUse hook 阻塞 + 远程回填） |
...
- ❌ 不是任何 Agent CLI 的 fork、不修改它们的二进制，只是 hook + 协议适配层。
- ❌ 不替你写代码，只是把已经能跑的 Coding Agent「延伸到手机上」。
...
| 版本  | 状态      | 关键能力                                                                                                                |
| ----- | --------- | ----------------------------------------------------------------------------------------------------------------------- |
| v0.1  | ✅ 已发  | 聊天 UI / 文件预览 / 临时分享 / 目录选择器（首发仅适配 Claude Code）                  |
| v0.2  | 🗓️ 开发中 | APK ntfy 长连接 + 通知按钮、iOS App（Bark 推送替代 ntfy） |
| v0.3  | 🗓️ 设计 | **AgentDriver 抽象层** + **Codex CLI 适配器** + **OpenCode 适配器**                  |
| V0.4 | 🚧 计划   |                                                           |
| v0.5 | 🚧 计划   |                                                           |
| v0.6 | 🚧 计划   |                                                           |
| v0.7 | 🚧 计划   |                                                           |
